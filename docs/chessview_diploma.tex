...
Контроль времени реализован в доменной модели игры. При ходе создается clock snapshot, затем активному игроку начисляется increment. Фоновый монитор активных игр находится в `backend/domains/game/presentation/runtime.py`; он проверяет таймауты и disconnect grace. Координация монитора выполняется через Redis lock `lock:game-monitor`, но нагрузочное тестирование нескольких backend-инстансов не проводилось.
...
Матчмейкинг & WebSocket queue, time controls, game creation & Реализовано через Redis-backed queue\\ \hline
...
Shop & Marketplace/wallet API and profile coin balance & Backend-backed demo surface\\ \hline
Clubs & Club/member API, ownership, visibility, searchable listings & Backend-backed demo surface\\ \hline
...
Ограничения не снижают ценность проекта, а показывают границы текущей инженерной версии. ChessView рассчитан на локальный или single-instance запуск. Matchmaking queue, WebSocket presence/rooms, cross-instance fanout и game monitor coordination используют Redis-backed механизмы (`backend/domains/matchmaking/application/services.py`, `backend/shared/ws_manager.py`, `backend/domains/game/presentation/runtime.py`). Uploaded media хранится локально. Для горизонтального масштабирования всё еще потребуются load balancer, object storage для media-файлов и отдельное нагрузочное тестирование.
